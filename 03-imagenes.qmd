...
## Segmentación (discretización) de datos Raster
...
# Re-clasifica los valores de los píxeles
ndvi_clases[np.where((ndvi_clases > 0.5))] = 2
ndvi_clases[np.where((ndvi_clases <= 0.5))] = 1
...
::: {.callout-important}
Es crucial mantener el orden correcto al reclasificar los valores de los píxeles. Si primero asignamos `[np.where(ndvi_clases <= 0.5)] = 1` y luego `[np.where(ndvi_clases > 0.5)] = 2`, los valores ya reclasificados en la primera condición serán también considerados en la segunda, lo que resultará en una única clase.
:::
...
lista_geo.plot(ax = ax,
               color = "green")
ax.set_title("NDVI vectrorial en Valdivia")
plt.show()
```
...
1.  Descargue el siguiente [set de datos](https://figshare.com/ndownloader/files/50335344). Ests son dos imágenes *Sentinel* de Viña del Mar antes y después de los incendios ocurridos en Febrero de este año. Las bandas entregadas son: B1, B2, B3, B4, B5, B6, B7, B8, B8a, B9, B11, B12.

2.  A través de *Rasterio* y *NumPy* calcule *NDVI*, *BAI* a ambas imágenes.

3.  Calcule el *NBR* y los derivados de este índice para ambas imágenes.

4.  A través de la @tbl-dnbr-severity, segmente el raster de *dNBR* y exportelo como un archivo `.tif`.

5.  Vectorice su raster segmentado y exportelo como archivo `.shp`.

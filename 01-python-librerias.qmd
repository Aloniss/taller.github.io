...
En el caso de que una correcta ejecución dentro de un *script* dentro *Spyder*, debe realizar una serie de pasos. A continuación, explicaremos un ejemplo de carga y visualización de datos. Este tendrá los siguientes pasos:
...
La principal herramienta para visualizar datos dentro de *Python* es *Matplotlib*, el cual, a través de su módulo `pyplot` permite graficar datos con estructura tabular. En el caso de visualización de rasters, el paquete *earthpy*, l otorga facilidades para mostrarlos y poder realizar composiciones de color.
...
Así, se debería llegar al siguiente resultado:
```{python}
#| eval: true
#| echo: false
#| fig-align: center
...
1.   Cargue puntos de ignicion (link) y plotee.

2.   Consulta por region (tabla)

3.   Consulta por region (gpd, espacial), note si hay diferencias.
